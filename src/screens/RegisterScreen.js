...
// ─────────────────────────────────────────────────────────────────────────────
// New user registration — 3 steps:
//   Step 1: Email + Password
//   Step 2: Personal info (name, age, gender, height, weight)
//   Step 3: Medical info (diseases, allergies)
//
// On success → POST /api/auth/register → save token + profile → go to Home
// ─────────────────────────────────────────────────────────────────────────────
...
export default function RegisterScreen({ navigation }) {
 ...
 // Add these with your other useState declarations at the top
  ...
  // ── Toggle chip (tap to select / deselect) ──────────────────────────────────
    ...
    try {
      ...
      // });
        const result = await registerUser({
          ...
          diseases,
          allergies,
          activity_level: activityLevel || null,   // null if not selected — field is optional
          dietary_pref:   dietaryPref   || null,
        });

      // Save token + profile to phone storage
      await saveSession(result.token, result.profile);

      // Go to Home — replace so user cannot go back to register
        ...
        {[1,2,3].map(i => (
          <View
            ...
            style={[
              styles.strengthBar,
              password.length >= i * 3 && {
                backgroundColor: password.length >= 9 ? COLORS.safe
                  : password.length >= 6 ? COLORS.caution
                  : COLORS.avoid
              }
            ]}
          />
    ...
    <View>
      ...
      <View style={styles.chipRow}>
        {['Omnivore', 'Vegetarian', 'Vegan', 'Pescatarian'].map(d => (
          <TouchableOpacity
            key={d}
            style={[styles.chip, dietaryPref === d && styles.chipActive]}
            onPress={() => setDietaryPref(d)}
          >
            <Text style={[styles.chipText, dietaryPref === d && styles.chipTextActive]}>
              {d}
            </Text>
          </TouchableOpacity>
        ))}

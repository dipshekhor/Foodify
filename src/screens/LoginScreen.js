...
// Email + password → POST /api/auth/login → save token → go to Home
//
// Also handles the case where token expired after 30 days:
// User is redirected here automatically by App.js
// ─────────────────────────────────────────────────────────────────────────────
...
export default function LoginScreen({ navigation }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // Save token + profile to AsyncStorage
      // This also resets the 30-day timer from today
      await saveSession(result.token, result.profile);

      // Navigate to Home — replace so back button doesn't go back to Login
      ...
      >
        ...
        >
          ...
          <Text style={styles.subtitle}>
            Sign in to continue checking your food safety.
          </Text>
